...
class AnalyzeResult {
  ...
  InterfaceElementSerializer? fromElementRef(InterfaceElementRefSerializer fromRef) {
    ...
    if (fromRef.jsonType == RefJsonType.internalElement) {
      final fileResult = files.firstWhere((e) => e.filePath == fromRef.source);
      element =
  ...
  TypeAliasElementSerializer? fromTypeAliasRef(TypeAliasElementRefSerializer fromRef) {
    final ref = int.parse(fromRef.ref.substring(1));
    if (aliasRef.containsKey(ref)) {
      return aliasRef[ref];
    }
    return null;
  }
