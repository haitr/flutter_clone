...
FutureOr<void> afterGenerated({List<String>? generatedPaths}) async {
  ...
  map.forEach((category, filePath) {
    final file = createIfNeeded(['riverpod_$category.dart'], outputDirectory: output);
    //
    final emitter = DartEmitter(orderDirectives: true, useNullSafetySyntax: true);
    var library = Library((builder) {
      builder.name = 'riverpod_widget';
      builder.directives.addAll(filePath.map((e) => Directive.import(e)));
      builder.directives.add(Directive.export('package:riverpod_widget/riverpod_widgets.dart'));
      builder.directives.add(Directive.export('wrapper.dart', show: ['BindExt', 'Wrapper', '\$']));
      filePath
